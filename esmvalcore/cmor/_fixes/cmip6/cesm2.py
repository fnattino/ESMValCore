...
class Msftmz(Fix):
    ...
    @staticmethod
    def transform_region_coord(coord: iris.coords.DimCoord)\
            -> iris.coords.AuxCoord:
        """transform a DimCoord with indexes as points to AuxCoord with names
        as points.
        Parameters
        ----------
        ...
        lookup = {int(p[1]): p[0] for p in
                  map(lambda x: x.split("="),
                      coord.attributes['requested'].split(",")
                      )
                  }
    ...
    def fix_metadata(self, cubes):
        """Fix for region DimCoord with indexes as points to AuxCoord with
        names as points.
        Parameters
        ----------
        cubes: iris CubeList
